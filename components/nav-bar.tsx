...
export function NavBar() {
  ...
  const recentNotifications = home?.user.notifications.slice(0, 2) ?? [];
          ...
          {popoverOpen && (
            <div className="absolute right-0 top-[calc(100%+0.65rem)] w-[224px] overflow-hidden rounded-[1.6rem] border border-white/[0.18] bg-[#070a11] text-white shadow-[0_30px_90px_rgba(0,0,0,.78),0_0_58px_rgba(216,255,62,.16)]">
              <div className="divide-y divide-white/[0.08] px-2 py-1 pb-4">
                {isSignedIn ? (
                  <Link href="/me" onClick={() => setPopoverOpen(false)} className="group flex min-h-12 items-center justify-between px-2.5 py-3 text-sm font-semibold text-white/76 transition hover:bg-white/[0.045]">
                  ...
                  <button type="button" onClick={() => openAuth("register")} className="group flex min-h-12 w-full items-center justify-between px-2.5 py-3 text-left text-sm font-semibold text-white/76 transition hover:bg-white/[0.045]">
                    <span className="flex items-center gap-2">
                      <UserRound className="h-4 w-4 text-volt/80" />
                      个人主页
                    </span>
                    <span className="text-[10px] font-bold uppercase tracking-[0.12em] text-volt/70">登录后进入</span>
                  </button>
                )}
                ...
                <section className="px-2.5 py-3">
                  ...
                  >
                    ...
                    <span className="inline-flex items-center gap-1 rounded-full bg-white/[0.055] px-2 py-1 text-[10px] font-bold text-white/42">
                    ...
                    <div className="mt-3 grid gap-2">
                      {isSignedIn && recentNotifications.length
                        ? recentNotifications.map((item) => (
                            <Link key={item.id} href="/me" onClick={() => setPopoverOpen(false)} className="block px-0 py-2 text-left transition hover:bg-white/[0.045]">
                              <p className="truncate text-xs font-semibold text-white/72">{item.title}</p>
                      ...
                      {isSignedIn && !recentNotifications.length && upcomingReminders.length
                        ...
                        : null}
                      {!isSignedIn || (!recentNotifications.length && !upcomingReminders.length) ? (
                        <button type="button" onClick={() => (isSignedIn ? setPopoverOpen(false) : openAuth("login"))} className="w-full px-0 py-3 text-left transition hover:bg-white/[0.045]">
                          <p className="text-xs font-semibold text-white/72">比赛提醒</p>
              ...
              {isSignedIn ? (
                <div className="px-2 pb-4">
                  <button type="button" onClick={logout} className="inline-flex h-10 w-full items-center justify-center gap-2 rounded-full bg-white/[0.055] text-xs font-semibold text-white/72 ring-1 ring-white/[0.1] transition hover:bg-white/[0.08] hover:text-white">
                ...
                <div className="grid grid-cols-2 gap-2 px-2 pb-4">
                  <button type="button" onClick={() => openAuth("login")} className="inline-flex h-10 items-center justify-center gap-2 rounded-full bg-white/[0.055] text-xs font-semibold text-white/72 ring-1 ring-white/[0.1] transition hover:bg-white/[0.08] hover:text-white">
                    <LogIn className="h-3.5 w-3.5" />
                    登录
                  </button>
                  <button type="button" onClick={() => openAuth("register")} className="inline-flex h-10 items-center justify-center gap-2 rounded-full bg-volt text-xs font-bold text-black shadow-[0_0_24px_rgba(216,255,62,.18)] transition hover:scale-[1.02]">
